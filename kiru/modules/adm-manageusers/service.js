...
function($http,$cookieStore,$rootScope,$timeout){
    ...
    service.addUser = function(org_name,token,to,callback){
        ...
        $http.get('connection.properties').then(function (response) {
            var posturl = response.data.authservice + ':'+ response.data.authentication +'/authentication/invite';
            var data={
    ...
    service.remove = function(email,token,callback){
        ...
        $http.get('connection.properties').then(function (response) {
            var posturl = response.data.authservice + ':'+ response.data.authentication +'/authentication/remove';
            var data={
                email:email,
                token:token
            }
